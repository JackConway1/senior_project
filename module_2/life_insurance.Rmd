...
\]
In this case, we let $b_{k+1} = 1$ and $n \rightarrow \infty$. In practice, we do not have life tables that extend to infinity. The computation of whole-life insurance closely resembles n-year term insurance but we utilize the table to the extent of the number of years that the table covers. An example utilizing software for a computation using an entire life table is covered in the next module.
...
```{r}
data(soa08Act)
exn(object = soa08Act, x = 20)
```
So, we expect a 10 year old to live approimately 63 more years based off of this life table.
