...
We can simulate this with option packages in R.
...
```{r}
stock.price.tree <- function(stock=100,rfree=.08,lvls=10,vol=.3,time=0.01)
{
    per <- time/(lvls-1);                   
    sq.per <- sqrt(per)                 
    intdelta <- exp(rfree*per)        
    up <- exp(vol*sq.per)               
    dwn <- 1/up                         
    ...
    for (rows in 1:lvls)
    {
        ...
        trow <- (up)^(0:(rows-1))*(dwn)^((rows-1):0) 
        trow <- stock*trow*intdelta^(rows-1)     
        tree[rows,1:rows] <- trow                
    }
tree
}
tail(stock.price.tree())
```
...
knitr::include_graphics("optiongreeks.png")
```
