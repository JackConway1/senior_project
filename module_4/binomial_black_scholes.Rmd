...
\qquad The **Binomial option pricing model** is a simple way to price options by assuming no arbitrage. Meaning no investment opportunities exist that require zero dollars of investment and provide positive yield. We will demonstrate the idea of no arbitrage pricing in the following example.

Consider a stock worth 100 at time 0. At time 1 the stock price is either 200 or 50. We will assume a one-period risk free rate: r = 11%.  Our goal is to find the price of a call option with strike price 150 and expiry date at time 1. Remember that a call is an option to buy an asset at the designated strike price at a future time. At time one if the call is exercised, the payoff is either 50 (200-150) or 0 (call won't be exercised at price 50). We will denote the price of the call option at strike price 150 over one period of time as C(150,1).
...
So suppose at time 0 we buy x shares of stock (at price 100x) and y call options (at price C(150,1)y). Then our value at time 1 is either 200x +50y if the stock price is 200. If the stock price is 50 at time one than the value is equal to 50x. Now suppose we choose y so that our payoff is equal.
...
At time 1 the share value that we lost from selling it will be equal to -50 at stock price 50. The option will not be exercised and the invested money will be worth 50. So the value of the portfolio at time 1 will be 0.  

If the stock price is 200 than the value lost from selling the share will be equal to -200. The options will be exercised for a payoff of 150 ((200-150)\*3) and the invested money will be worth 50. So the value of the portfolio at time 1 will be 0.
...
We can see here that we have a positive payoff at time 0 for no cost since the payoff at time 1 will be 0 whether the stock is worth 50 or 200. Similarly if the call option was more expensive than 18.32 we would take the opposite postion. If we followed the same process as before, but instead we buy 1 share, sell 3 options, and borrow from the bank we would have a positive payoff at 0 and value of 0 at time 1.  

\qquad This is the idea of arbitrage that there is an opportunity for arbitrage if there is a positive return with no risk of loss. 
...
So the fair price of a 1-year call option at the strike price of 55 will be \$2.99. Consider a more simple example where we observe 10000 changes in the price of a stock. If we had observed 6500 cases where the price had went up and 3500 where the price of the stock went down, then we would estimate the probability of a stock increasing in price to be $\frac{6500}{10000}= 0.65$. This is the general idea that we want to capture with our estimation of increase.  
...
We can use the quantmod package in R to get up to date estimates on recent stocks. For example if we were interested in calculating the volatility of APPL stock we could use this package to estimate it.
...
\qquad This limiting process of the binomial formula converges to the Black-Scholes model discussed in section 2 below. This assumes a lognormal distribution of prices and relies on the central limit theorem to show that "as the number of periods into which the fixed length of time to expiration is divided approaches infinity, the probability that the standardized continuously compounded rate of return of the stock through the expiration date is less than or equal to the number z approaches the probability under a standard normal distribution"(Journal of Financial Econmics 7(1979)). 

\qquad This happens because if we take the expected value of the ratio of stock prices, as the time period length becomes close to zero our expected value by the central limit theorem is $\mu t$. Similarly, if we calculate the variance of this ratio after working out the algebra we get $\sigma ^2 t$.

\qquad To think about this let $S_T$ be the price of a stock at time t. In our model of stock prices, we are going to assume that the rate of return on the stock is normally distributed. Suppose $\alpha$ is the continuously compounded annual rate of return. If there were no randomness then we would have $S_T = S_0 e^{\alpha t}$. Since $S_T$ is random we have $S_T = S_0 e^{A t}$ where A is a normally distributed ranom variable, so the ratio is lognormally distributed.

We can simulate this small change to show a nearly continuous spectrum of possible asset prices.

```{r}
upordown <- c(1.01, 0.99)
sample.path <- NULL
value <- NULL
